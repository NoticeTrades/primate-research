...
export async function initDb() {
  ...
  await sql`
    CREATE TABLE IF NOT EXISTS live_trades (
      ...
      exit_quantity INTEGER DEFAULT 0 CHECK (exit_quantity >= 0 AND exit_quantity <= quantity),
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_live_trades_status ON live_trades ((CASE WHEN exit_quantity < quantity THEN 0 ELSE 1 END), created_at DESC)
  `;
