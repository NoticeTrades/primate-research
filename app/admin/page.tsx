...
export default function AdminPage() {
  ...
  // Index market structure state
  const [indexStructures, setIndexStructures] = useState<{ symbol: string; daily_structure: string; weekly_structure: string; monthly_structure: string | null }[]>([]);
  const [indexStructuresLoading, setIndexStructuresLoading] = useState(false);
  const [indexStructuresLoaded, setIndexStructuresLoaded] = useState(false);
  ...
  const fetchIndexStructures = async () => {
    setIndexStructuresLoading(true);
    setIndexUpdateStatus('');
    try {
      const res = await fetch(`/api/admin/indices?secret=${encodeURIComponent(secret)}`);
      const data = await res.json();
      if (res.ok) {
        setIndexStructures(data.structures || []);
        setIndexStructuresLoaded(true);
      }
    } catch {
      // ignore
    } finally {
      setIndexStructuresLoading(false);
    }
  };

  const handleEditIndex = (index: { symbol: string; daily_structure: string; weekly_structure: string; monthly_structure: string | null }) => {
    setEditingIndex(index.symbol);
    setEditDaily(index.daily_structure || '');
    setEditWeekly(index.weekly_structure || '');
    setEditMonthly(index.monthly_structure || '');
    setIndexUpdateStatus('');
  };
  ...
  const handleSaveIndex = async (symbol: string) => {
    if (!editDaily.trim() || !editWeekly.trim()) {
      setIndexUpdateStatus('Daily and Weekly structures are required');
      return;
    }

    setIndexUpdateStatus('Saving...');
    try {
      const res = await fetch('/api/admin/indices', {
        ...
        body: JSON.stringify({
          secret,
          symbol,
          daily: editDaily.trim(),
          weekly: editWeekly.trim(),
          monthly: editMonthly.trim() || null,
        }),
      });
      const data = await res.json();
      if (res.ok) {
        setIndexUpdateStatus('✅ Market structure updated successfully!');
        await fetchIndexStructures();
        setTimeout(() => {
          setEditingIndex(null);
          setEditDaily('');
          setEditWeekly('');
        ...
        setIndexUpdateStatus(`Error: ${data.error || 'Failed to update'}`);
      }
    } catch (error: any) {
  ...
  const fetchIndexCharts = async () => {
    setIndexChartsLoading(true);
    setChartUploadStatus('');
    try {
      const url = chartFilterSymbol
        ? `/api/admin/index-charts?secret=${encodeURIComponent(secret)}&symbol=${chartFilterSymbol}`
        : `/api/admin/index-charts?secret=${encodeURIComponent(secret)}`;
      const res = await fetch(url);
      const data = await res.json();
      if (res.ok) setIndexCharts(data.charts || []);
      else setChartUploadStatus(data.error || 'Failed to load charts');
      setIndexChartsLoaded(true);
    } catch {
      setChartUploadStatus('Failed to load charts');
    } finally {
      setIndexChartsLoading(false);
    }
  };

  const handleUploadIndexChart = async () => {
    if (!chartFile) {
      setChartUploadStatus('Select an image first (PNG, JPEG, WebP, GIF).');
      return;
    }
    ...
    try {
      ...
      form.append('symbol', chartUploadSymbol);
      ...
      const data = await res.json();
      if (res.ok) {
        setChartUploadStatus('✅ Chart uploaded. It will appear on the ' + chartUploadSymbol + ' index page.');
        setChartFile(null);
        setChartUploadTitle('');
        setChartUploadDate('');
        ...
        setChartUploadStatus(data.error || 'Upload failed');
      }
    } catch (e: any) {
      setChartUploadStatus(e?.message || 'Upload failed');
    } finally {
      setChartUploading(false);
    }
  };

  const handleDeleteIndexChart = async (id: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) await fetchIndexCharts();
      else setChartUploadStatus(data.error || 'Delete failed');
    } catch {
  ...
  const adminTabPanelsContent = (
    <div className="contents">
        ...
        {activeTab === 'indices' && (
          ...
          {/* Index Charts — show on index pages */}
          <div className="bg-zinc-900 border border-blue-500/20 rounded-xl overflow-hidden mb-8">
            <div className="px-6 py-4 border-b border-zinc-800 flex flex-wrap items-center justify-between gap-4">
              <h2 className="text-lg font-semibold">📈 Index Charts (All Supported Assets)</h2>
              <div className="flex items-center gap-3">
                <select
                  value={chartFilterSymbol}
                  onChange={(e) => setChartFilterSymbol(e.target.value as ChartSymbol | '')}
                ...
                <div className="flex flex-wrap gap-4 items-end">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setChartUploadSymbol(e.target.value as ChartSymbol)}
