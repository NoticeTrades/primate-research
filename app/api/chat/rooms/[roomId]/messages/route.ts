...
export async function GET(
  ...
  try {
    ...
    // Get messages (paginated for scalability). Requires reply_to_id column (run ALTER in Neon if missing).
...
export async function POST(
  ...
  try {
    ...
    // Insert message (requires reply_to_id column in DB; run ALTER in Neon if missing)
    ...
    if (msgRow.reply_to_id) {
      ...
      `;
      const replyList = Array.isArray(replyRow) ? replyRow : (replyRow as { rows?: { username: string; reply_to_text: string }[] })?.rows ?? [];
      if (replyList.length > 0) {
        const r = replyList[0] as { username: string; reply_to_text: string };
