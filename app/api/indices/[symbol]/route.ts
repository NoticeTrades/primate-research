        ...
        if (data.status === 'ok' && data.price) {
          ...
          if (quoteRes.ok) {
            ...
            if (quote.status === 'ok') {
              ...
              // Daily change = vs previous close ("closed up/down X%")
              const change = price - prevClose;
              const changePercent = prevClose > 0 ? (change / prevClose) * 100 : 0;
              
              console.log(`[Twelve Data] Successfully fetched data for ${symbol} (${twelveSymbol}): price=${price}, change=${changePercent.toFixed(2)}%`);
  ...
  try {
    ...
    if (quoteRes.ok) {
      ...
      if (q) {
        ...
        let changeVal = 0;
        let changePct = 0;
        if (prevClose > 0 && regularMarketPrice > 0) {
          ...
          changePct = typeof rawChangePercent === 'number' && !Number.isNaN(rawChangePercent)
            ? rawChangePercent
            : (prevClose ? (changeVal / prevClose) * 100 : 0);
        }
        
        // Try to get better price from chart API if needed (keep daily change = vs previous close)
        if ((changeVal === 0 || prevClose === 0) && typeof regularMarketPrice === 'number') {
          try {
            ...
            if (chart5Res.ok) {
              ...
              if (meta && quote) {
                ...
                const prevCloseFromChart = meta?.previousClose ?? (closeArr.length >= 2 ? closeArr[closeArr.length - 2] : null) ?? prevClose;
                if (prevCloseFromChart > 0 && currentPrice > 0) {
                  changeVal = currentPrice - prevCloseFromChart;
                  changePct = (changeVal / prevCloseFromChart) * 100;
                  regularMarketPrice = currentPrice;
                  if (prevClose === 0) prevClose = prevCloseFromChart;
                }
              }
            }
          } catch {
    ...
    // Fetch historical data for seasonality (15 years for better accuracy)
    let seasonalityData: { month: number; avgReturn: number; positiveMonths: number; totalMonths: number }[] = [];
    try {
      const histUrl = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?interval=1mo&range=15y`;
      ...
      if (histRes.ok) {
        ...
        if (Array.isArray(closes) && Array.isArray(timestamps) && timestamps.length === closes.length) {
          // Group by month and calculate average returns (month-over-month)
    ...
    // Seasonal key terms with actual stats from seasonality (month 0=Jan .. 11=Dec)
    const monthMap = Object.fromEntries(seasonalityData.map((s) => [s.month, s]));
    const getAvgForMonths = (months: number[]) => {
      const values = months.map((m) => monthMap[m]?.avgReturn).filter((v) => typeof v === 'number');
      if (values.length === 0) return null;
      return values.reduce((a, b) => a + b, 0) / values.length;
    };
    const seasonalKeyTerms = [
      { term: 'January Effect', months: [0], description: 'Historically strong month; institutional flows and new-year positioning.', avgReturn: getAvgForMonths([0]) },
      { term: 'Santa Rally', months: [11], description: 'December often sees a year-end rally into the holidays.', avgReturn: getAvgForMonths([11]) },
      { term: 'Sell in May', months: [4, 5], description: 'April–May can mark a seasonal soft patch before summer.', avgReturn: getAvgForMonths([4, 5]) },
      { term: 'Summer Markets', months: [6, 7, 8], description: 'June–August: typically lower volume and sometimes range-bound (summer doldrums).', avgReturn: getAvgForMonths([6, 7, 8]) },
      { term: 'September Effect', months: [8], description: 'Historically one of the weaker months for equities.', avgReturn: getAvgForMonths([8]) },
