...
export async function initDb() {
  ...
  `;
  await sql`
    ALTER TABLE users ADD COLUMN IF NOT EXISTS trade_sms_consent_at TIMESTAMP WITH TIME ZONE
  `;
  await sql`
    ALTER TABLE users ADD COLUMN IF NOT EXISTS trade_sms_opt_out_at TIMESTAMP WITH TIME ZONE
  `;
  ...
  // Index charts (admin-uploaded charts for ES/NQ/YM ticker pages)
  await sql`
    CREATE TABLE IF NOT EXISTS index_charts (
      id SERIAL PRIMARY KEY,
      symbol TEXT NOT NULL,
  ...
  `;
  await sql`
    CREATE INDEX IF NOT EXISTS idx_index_charts_symbol ON index_charts(symbol)
  `;
  await sql`
